...
/// Parses the trainee list page from `https://umamusu.wiki/Game:List_of_Trainees`
pub fn parse_uma_index(html: &str) -> Vec<UmaIndexEntry> {
    ...
    for row in document.select(&row_sel) {
        match parse_uma_index_row(row, &td_sel, &img_sel, &a_sel) {
            UmaIndexRow::Parsed(entry) => entries.push(entry),
            UmaIndexRow::SkippedJp => skipped_jp += 1,
            UmaIndexRow::SkippedParse => skipped_parse += 1,
        }
    }
...
fn parse_uma_index_row(
    ...
    if cells.len() < 5 {
        return UmaIndexRow::SkippedParse;
    }

    if cells[4].value().attr("data-sort-value").is_none() {
        return UmaIndexRow::SkippedJp;
    }
    ...
    UmaIndexRow::Parsed(UmaIndexEntry { id, name })
}
...
mod tests {
    ...
    fn parses_valid_en_row() {
        ...
        let entries = parse_uma_index(&html);
